...
const projects = [
  {
    ...
    description: 'A flexible identity for a neighborhood space where food, conversation, and good ideas overlap.',
    tone: 'sage',
  ...
  },
  {
    ...
    type: 'Editorial / 2023',
    description: 'An independent print journal about the rituals we keep when no one is watching.',
    tone: 'tomato',
    tags: ['Art direction', 'Print', 'Photography'],
  },
  {
    id: '03',
    title: 'Soft Systems',
    type: 'Digital product / 2023',
    description: 'Making a complicated climate tool feel less like a spreadsheet and more like a shared map.',
    tone: 'mist',
    tags: ['UX direction', 'Product', 'Motion'],
  },
];

const capabilities = [
  ['01', 'Positioning', 'Finding the clear, human sentence at the center of a good idea.'],
  ['02', 'Identity systems', 'Building visual languages with enough structure to grow and enough character to be remembered.'],
  ['03', 'Digital experiences', 'Designing websites and products that reward curiosity, not just clicks.'],
  ['04', 'Creative direction', 'Gathering the right people, references, and energy to make the work feel inevitable.'],
];
...
function Home() {
  ...
  const copyEmail = async () => {
    await navigator.clipboard?.writeText('hello@marisol.studio');
    setCopied(true);
    ...
    <main className="portfolio-noise min-h-[100dvh] bg-[var(--paper)] text-[var(--ink)]">
      <header className="fixed left-0 right-0 top-0 z-20 border-b border-[var(--ink)]/10 bg-[var(--paper)]/90 backdrop-blur-md">
        <div className="mx-auto flex h-[76px] max-w-[1240px] items-center justify-between px-5 sm:px-8 lg:px-12">
          <a href="#top" onClick={closeMenu} className="group flex items-center gap-3" data-testid="link-brand">
            <span className="grid h-9 w-9 place-items-center rounded-full bg-[var(--ink)] font-display text-lg text-[var(--paper)]">M</span>
            <span className="text-sm font-semibold tracking-[-.02em]">Marisol Vale<span className="text-[var(--tomato)]">.</span></span>
          </a>
          <nav className={`${menuOpen ? 'flex' : 'hidden'} absolute left-4 right-4 top-[84px] flex-col gap-6 rounded-2xl border border-[var(--ink)]/15 bg-[var(--paper)] p-6 shadow-lg md:static md:flex md:flex-row md:items-center md:gap-8 md:border-0 md:bg-transparent md:p-0 md:shadow-none`} aria-label="Main navigation">
            ...
            <a href="mailto:hello@marisol.studio" className="flex items-center gap-2 text-sm font-semibold md:ml-2" data-testid="link-email">Let&apos;s talk <ArrowUpRight size={15} /></a>
          </nav>
      ...
      <section id="top" className="relative mx-auto flex min-h-[820px] max-w-[1240px] items-center px-5 pb-20 pt-36 sm:px-8 lg:min-h-[900px] lg:px-12">
        <div className="relative z-10 max-w-[760px]">
          ...
          <h1 className="reveal reveal-delay-1 mt-8 font-display text-[clamp(4.5rem,12vw,10.8rem)] leading-[.84] tracking-[-.075em]">
            ...
            a pulse<span className="text-[var(--tomato)]">.</span>
          </h1>
          <div className="reveal reveal-delay-2 mt-10 flex max-w-[560px] flex-col gap-6 sm:ml-[18%] sm:flex-row sm:items-end">
            <p className="max-w-[360px] text-lg leading-relaxed text-[var(--ink)]/72">
              I&apos;m Marisol Vale, a creative director and designer making identities, digital experiences, and other good reasons to look twice.
            </p>
      ...
      <div className="overflow-hidden border-y border-[var(--ink)] bg-[var(--tomato)] py-4 text-[var(--ink)]">
        <div className="flex min-w-max items-center gap-8 font-display text-2xl italic">
          <span>Good work asks better questions</span><span className="text-lg">+</span><span>Good work asks better questions</span><span className="text-lg">+</span><span>Good work asks better questions</span>
        </div>
      </div>
      ...
      <section id="work" className="mx-auto max-w-[1240px] px-5 py-28 sm:px-8 lg:px-12 lg:py-36">
        <div className="reveal flex flex-col justify-between gap-8 md:flex-row md:items-end">
          <div><SectionLabel number="01">The good stuff</SectionLabel><h2 className="mt-6 max-w-[700px] font-display text-5xl leading-[.95] tracking-[-.055em] sm:text-7xl">A few things I&apos;ve<br /><em className="text-[var(--tomato)]">helped into the world.</em></h2></div>
          <p className="max-w-[260px] text-sm leading-relaxed text-[var(--ink)]/60">A rotating edit of collaborations, experiments, and work made with people I like.</p>
        </div>
      ...
      <section className="bg-[var(--ink)] text-[var(--paper)]">
        <div className="mx-auto max-w-[1240px] px-5 py-28 sm:px-8 lg:px-12 lg:py-36">
          <div className="reveal grid gap-12 md:grid-cols-12">
            <div className="md:col-span-4"><SectionLabel number="02">How I help</SectionLabel><p className="mt-8 max-w-[260px] text-sm leading-relaxed text-[var(--paper)]/60">From the first hunch to the last pixel, I bring shape and feeling to ambitious ideas.</p></div>
      ...
      <section id="about" className="mx-auto max-w-[1240px] px-5 py-28 sm:px-8 lg:px-12 lg:py-36">
        <div className="grid gap-16 lg:grid-cols-12 lg:gap-8">
          ...
          <div className="reveal reveal-delay-1 lg:col-span-6 lg:col-start-7 lg:pt-16">
            <p className="font-display text-3xl leading-[1.12] tracking-[-.03em] sm:text-4xl">I believe the best design feels less like decoration and more like recognition — a small click of <em>“oh, that’s exactly it.”</em></p>
            <p className="mt-8 max-w-[500px] text-base leading-[1.8] text-[var(--ink)]/68">This is sample copy for Marisol Vale, an editable profile. Replace it with your own point of view, biography, and the details that make your practice yours. The strongest portfolio isn&apos;t a résumé in nicer clothes; it&apos;s an invitation into how you see.</p>
        ...
        <div className="reveal reveal-delay-2 mt-28 grid border-t border-[var(--ink)]/20 pt-8 sm:grid-cols-3">
          <div><p className="eyebrow text-[var(--tomato)]">Now</p><p className="mt-3 text-sm">Independent creative director<br />& designer · New York / everywhere</p></div>
          <div className="mt-8 sm:mt-0"><p className="eyebrow text-[var(--tomato)]">Before</p><p className="mt-3 text-sm">Studio practice · Brand teams<br />Editorial side quests</p></div>
          <div className="mt-8 sm:mt-0"><p className="eyebrow text-[var(--tomato)]">Also</p><p className="mt-3 text-sm">Long walks · short stories<br />Very good playlists</p></div>
        </div>
      </section>
      ...
      <section className="border-y border-[var(--ink)]/15 bg-[var(--mist)]">
        <div className="mx-auto max-w-[1240px] px-5 py-24 sm:px-8 lg:px-12 lg:py-28">
          <div className="reveal grid gap-10 md:grid-cols-12 md:items-center">
            ...
            <blockquote className="md:col-span-8 md:col-start-5"><p className="font-display text-4xl leading-[1.06] tracking-[-.04em] sm:text-6xl">“The rare person who can zoom out to the big idea and still notice when the tiny things are off.”</p><footer className="mt-7 flex items-center gap-3 text-sm"><span className="h-8 w-8 rounded-full bg-[var(--tomato)]" /><span><strong>Sample collaborator</strong><br /><span className="text-[var(--ink)]/60">Replace this with a real voice from your practice.</span></span></footer></blockquote>
          </div>
        </div>
      </section>
      ...
      <section id="contact" className="mx-auto max-w-[1240px] px-5 py-28 sm:px-8 lg:px-12 lg:py-40">
        <div className="reveal relative overflow-hidden rounded-[2rem] bg-[var(--tomato)] px-7 py-14 sm:px-14 sm:py-20">
          <div className="relative z-10 max-w-[780px]"><SectionLabel number="05">Over to you</SectionLabel><h2 className="mt-7 font-display text-6xl leading-[.88] tracking-[-.06em] sm:text-8xl">Have a good<br /><span className="ml-[.35em]">feeling?</span></h2><p className="mt-8 max-w-[420px] text-base leading-relaxed text-[var(--ink)]/75">Tell me what you&apos;re making, what&apos;s stuck, or what you can&apos;t stop thinking about. I&apos;d love to hear it.</p><div className="mt-9 flex flex-wrap items-center gap-5"><a href="mailto:hello@marisol.studio" className="inline-flex items-center gap-3 rounded-full bg-[var(--ink)] px-6 py-3.5 text-sm font-semibold text-[var(--paper)] transition-transform hover:-translate-y-1" data-testid="button-send-email">hello@marisol.studio <ArrowUpRight size={17} /></a><button type="button" onClick={copyEmail} className="inline-flex items-center gap-2 text-sm font-semibold" data-testid="button-copy-email">{copied ? <><Check size={16} /> copied</> : <><Mail size={16} /> copy email</>}</button></div></div>
      ...
      <footer className="border-t border-[var(--ink)]/15">
        <div className="mx-auto flex max-w-[1240px] flex-col gap-8 px-5 py-8 sm:px-8 md:flex-row md:items-center md:justify-between lg:px-12">
          <div className="flex items-center gap-3"><span className="grid h-8 w-8 place-items-center rounded-full bg-[var(--ink)] font-display text-sm text-[var(--paper)]">M</span><span className="text-sm">Marisol Vale<span className="text-[var(--tomato)]">.</span></span></div>
          <p className="font-mono-custom text-[10px] uppercase tracking-[.12em] text-[var(--ink)]/50">A sample identity · edit freely · © 2025</p>
